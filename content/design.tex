...
When designing a privacy preserving solution for IoT, we must aim for a series of objectives that guarantee its integrity and security. Our requirements are:

\begin{itemize}
	\item \textit{Security}: the solution should not compromise the security of the device's credentials.
...
In this section we describe the computation offloading carried out by the IoT device. The steps the device will perform in any kind of interaction are:

\begin{itemize}
	\item Communication with P2ABCE actor
	
	The IoT device, acting as a User, starts an interaction with another actor. If it is an Issuer, it will start the issuance process. If it is a Verifier, then it will provide a Presentation Policy for the device to proof in a privacy preserving way.  In Figure~\ref{fig:DelegationProving} we show an example of this last case.
